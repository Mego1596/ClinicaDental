...
@section('content')
<h1 style="text-align: center;"><strong>Detalles del Rol: {{$rol->name}}</strong></h1>
@if (count($errors) > 0)
...
<div class="form-group">
	...
	<input type="text" class="form-control" id="name" name="name" placeholder="Nombre del Rol" required value="{{$rol->name}}" disabled readonly>
</div>
<div class="form-group">
	<label for="slug">Identificador:</label>
	<input type="text" class="form-control" id="slug" name="slug" placeholder="Identificador" required value="{{$rol->slug}}" disabled readonly>
</div>

<div class="form-group">
	<label for="description">Descripcion:</label>
	<input type="text" class="form-control" id="description" name="description" placeholder="Descripcion" required value="{{$rol->description}}" disabled readonly>
</div>
						...
						<div class="col-sm-10">
							...
							<div class="form-check">
								<label class="form-check-label">
									<input type="checkbox" name="permission[]" value="{{$permiso->id}}" checked disabled readonly>
									{{$permiso->description}}
								</label>
							</div>
							@else
							<div class="form-check">
								<label class="form-check-label">
									<input type="checkbox" name="permission[]" value="{{$permiso->id}}" disabled readonly>
									{{$permiso->description}}
								</label>
							</div>
							@endif
...
<div class="d-flex justify-content-center">
	<a href="{{route('roles.index')}}" class="btn btn-sm btn-danger" style=""><i class="fa fa-arrow-circle-left"></i> Regresar</a>
</div>
@endsection